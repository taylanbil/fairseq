...
def main(args, init_distributed=False):
    ...
    train_meter.stop()
    logger.info('done training in {:.1f} minutes'.format(train_meter.sum))
...
def train(args, trainer, task, epoch_itr, max_update=math.inf):
    ...
    for samples in progress:
        ...
        valid_losses = validate_and_save(
            args, trainer, task, valid_subsets, epoch_itr, end_of_epoch,
        )
...
def validate_and_save(args, trainer, task, valid_subsets, epoch_itr, end_of_epoch):
    num_updates = trainer.get_num_updates()
    do_save = (
        (
